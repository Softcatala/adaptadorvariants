...
# a l'inspector
s/\b\(Gràfic en 3D»\|Posició del divisor horitzontal\|Posició del divisor vertical\|usar\) a l'inspector\([^[:alnum:]]\)/\1 en l'inspector\2/g
# a l'intèrpret
...
s/\b\(acceleròmetre\|desviació\|horitzontal\|mostra una etiqueta\|text plegat\|vertical\) a la vora\([^[:alnum:]]\)/\1 en la vora\2/
 #
s/\b\(representar fora del vídeo,\) a les vores\([^[:alnum:]]\)/\1 en la vores\2/g
# a la vostra
...
# al navegador
s/\b\(apareguin\|Bu&sca\|inici de sessió\|l'agenda i\|veure els fitxers de Krita\) al navegador\([^[:alnum:]]\)/\1 en el navegador\2/g
# al núvol
...
# al directori
s/\b\(addicionals\|adreces\|cap %1\|carpeta «gsc»\|cerca\|cercar\|cerquis\|configuració\|desarà\|disc\|emmagatzemats\|FIFO\|fitxer\|fitxer nou\|fitxers\|gsc»\|instal·leu\|mantenir\|nom\|ordres\|personalitzats\|registre\|romanen\|situa\|situa[rt]\|troba[rt]\|ubicat normalment\|uninstall`\) al directori\([^[:alnum:]]\)/\1 en el directori\2/g
...
# al filtre
s/\b\(Activa les expressions regulars\) al filtre\([^[:alnum:]]\)/\1 en el filtre\2/g
# al fitxer
...
# al gràfic
s/\b\(Mostra la zona d'enfocament crític\|Mostra les etiquetes «Màx.» i «Mín.»\|netejar el desordre, tant\) al gràfic\([^[:alnum:]]\)/\1 en el gràfic\2/g
# als jardins
...
# al pòdcast
s/\b\(Millores\) als pòdcast\([^[:alnum:]]\)/\1 en els pòdcast\2/g
# al port
...
s/\b\([Ee]specifiqueu un camí absolut\|introduir un patró d'intèrpret d'ordres\|[Mm]ostrar\|[Mm]ostrarà\|[Oo]culta els elements nous\|proporciona l'usuari\|s'introdueix\|se substituïxen els valors\|selecciona[rt]\|seleccionada\) al quadre\([^[:alnum:]]\)/\1 en el quadre\2/g
...
s/\b\(controls d'estirament\|cursor\|fitxer\|imatge\|Marca les estrelles»\|[Mm]ostra el fitxer\|[Mm]ostrar els vídeos\|mostrarà\|[Oo]bre\|pantalla d'estat\|ratolí\|rebudes\|s'obriran\|utilitzar\|utilitzarà\|veu\) al vis\(or\|ualitzador\)\([^[:alnum:]]\)/\1 en el visor\3/g
# al vostre
s/\b\(aplicable\|configurada\|configurades\|configurat\|configurats\|emmagatzemant una galeta\|instal·lat el &frameworks;\|instal·lats\|KDE\|referir-se a un recurs tal com un fitxer\|Responeu a missatges de text del telèfon\|tal com un fitxer\|troba\|Vegeu les notificacions del telèfon\) al vostre\([^[:alnum:]]\)/\1 en el vostre\2/g
# al wiki
