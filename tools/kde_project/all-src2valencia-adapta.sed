...
s/\b\([Ll]\)'\(\[\|\*\|\*\*\|\*\*\*\|[|&«]\|<[^<]\{1,\}>\|\)\(\.aac\|aac\|AAC\|\.aif\|\.aif[cf]\|AIFF\|ARJ\|AV1\|AVIF\|ELF\|EPub\|EXR\|MP3\|MYB\|Ogg\|Opus\|ORA\|XMPP\|XSL\)\(;\|\)\([^[:alnum:]]\)/\2\3\4\5/g
 # el -> la
s/\"\(\[\|\*\|\*\*\|\*\*\*\|[|&«]\|<[^<]\{1,\}>\|\)\([ACV]BR\|KatePart\|KPart\|QApplication\|VoIP\|WebEnginePart\)\(;\|\) \([^[:alnum:]]\)/\"La \1\2\3 \4/g
s/\"La \(\[\|\*\|\*\*\|\*\*\*\|[|&«]\|<[^<]\{1,\}>\|\)\([ACV]BR\|KatePart\|KPart\|QApplication\|VoIP\|WebEnginePart\)\(;\|\)\"/\"\1\2\3\"/g
s/\b\([Aa]\)l \(\[\|\*\|\*\*\|\*\*\*\|[|&«]\|<[^<]\{1,\}>\|\)\([ACV]BR\|KatePart\|KPart\|QApplication\|VoIP\|WebEnginePart\)\(;\|\)\([^[:alnum:]]\)/\1 la \2\3\4\5/g
s/\bEl \(\[\|\*\|\*\*\|\*\*\*\|[|&«]\|<[^<]\{1,\}>\|\)\([ACV]BR\|KatePart\|KPart\|QApplication\|VoIP\|WebEnginePart\)\(;\|\)\([^[:alnum:]]\)/La \1\2\3\4/g
s/\bel \(\[\|\*\|\*\*\|\*\*\*\|[|&«]\|<[^<]\{1,\}>\|\)\([ACV]BR\|KatePart\|KPart\|QApplication\|VoIP\|WebEnginePart\)\(;\|\)\([^[:alnum:]]\)/la \1\2\3\4/g
s/\b\([Dd]\)el \(\[\|\*\|\*\*\|\*\*\*\|[|&«]\|<[^<]\{1,\}>\|\)\([ACV]BR\|KatePart\|KPart\|QApplication\|VoIP\|WebEnginePart\)\(;\|\)\([^[:alnum:]]\)/\1e la \2\3\4\5/g
# #
# # # # # # # # # # # # # #
#
# entitats
...
s/&Desa com a\([^[:alnum:]]\)/Anomena i gu\&arda\1/g
    ...
    s/\bDesa com &a\([^[:alnum:]]\)/Anomena i gu\&arda\1/g
        ...
        s/\bDesa el \(catàleg\|perfil\) com a\([^[:alnum:]]\)/Anomena i guarda el \1\2/g
        ...
        s/\bDesa la \(programació\|seqüència de captura\|sessió d'observació\|versió nova\) com a\([^[:alnum:]]\)/Anomena i guarda la \1\2/g
...
# a Android
s/\b\(mostrar-les\) a Android\([^[:alnum:]]\)/\1 en Android\2/g
# a aquest(a)
...
# a cada
s/\b\(buida\|buida la memòria cau\) a cada\([^[:alnum:]]\)/\1 en cada\2/g
# a Cantor
...
# a invent
s/\b\(crear una petició de fusió\|GitLab\) a \(<[^<]\{1,\}>\|«\|\[\|\)invent\([^[:alnum:]]\)/\1 en \2invent\3/g
# a irc
s/\bunir-vos a #kate a irc\([^[:alnum:]]\)/unir-vos a #kate en irc\1/g
# a KAddressBook
...
# a les opcions
s/\b\(canvieu\|L'accés\|rotor»,\) a les \(<[^<]\{1,\}>\|«\|\[\|\)\([Oo]\)pcions\([^[:alnum:]]\)/\1 en les \2\3pcions\4/g
# a Marble
...
s/\b\(apareixerà en el menú CDV i\|establir la muntura\|estigui\|[Mm]illores\|mostra\|mostraran\|mostren\|Obre una imatge\|Utilitza la vista prèvia de la pila\) a l'eina\([^[:alnum:]]\)/\1 en l'eina\2/g
 #
...
# a la captura
s/\b\(exclou les decoracions\|exclou les ombres\|inclou el punter\|mostra't\|Per exemple,\|s'inclou\|s'inclouen\|tal com s'il·lustra\) a la captura\([^[:alnum:]]\)/\1 en la captura\2/g
# a la cara
...
s/\b\(falta\|funcionarà\|present\|troba\) a la majoria\([^[:alnum:]]\)/\1 en la majoria\2/g
# a la manera
...
# a la planificació
s/\b\(introducció\) a la planificació\([^[:alnum:]]\)/\1 en la planificació\2/g
# a la plataforma
...
# a la secció
s/\b\(activat\|clau i el nom\|com botons\|del sistema\|establir\|ID de la clau\|literal>\|seleccioneu la configuració preferida\|títol de l'element\) a la secció\([^[:alnum:]]\)/\1 en la secció\2/g
# a la segona
...
s/\b\(1 element\|1 elements\|[Cc]lic\|col·lisions de peces\|com un camp\|falten\|gràfic com\|[Hh]i ha un espai\|[Ii]nclou\|línia\|[Mm]ostra\|mostrar la barra d'estat\|[Pp]rem\|previsualització i\|propietat donada d'un element\|punts\|ressalten\|selecciona-les\|seleccionada\|veure la durada de cada esdeveniment\) a la taula\([^[:alnum:]]\)/\1 en la taula\2/g
 #
...
s/\(,\|»\|<\/b>\|)TeX\|abc\|actualitza automàticament\|alhora\|apareixerà\|artefacte\|bibitems\|canviar-ho\|canviar-la\|categories\|cel·la\|clic\|columnes\|conciliades\|contextual\|cppcheck»\|definides\|desar-la\|diaris\|disponibles\|elements\|els fitxers\|entrades\|[Ee]rror\|erronis\|esdeveniments\|espai\|esquerre\|esteu\|etiqueta\|etiquetes\|expansió\|festiu\|finalitzades\|fitxer\|flotants\|fracció\|gràfic\|hora actual\|horari\|hores\|i\|icones\|imatges\|línia\|llista\|llistaran\|mantindran\|manual\|manualment)\|marcar\|miniatures\|mostra\|Mostra el títols dels àlbums i els artistes\|mostrar\|mostraran\|mostren\|moveu el cursor\|navegar\|nombres\|normals\|nou\|objecte\|[Oo]bre\|obrir-l[ao]\|oculta\|operació\|ordenen\|part inferior\|part superior\|peces\|pendents\|podcast\|preferit\|projecte\|registre\|resource>\|resultat\|que\|s'utilitzarà\|s'utilitzarà per als esdeveniments\|seleccionada\|seleccionats\|Seleccioneu aquí el color de fons de les hores laborables\|Seleccioneu aquí el color de les hores laborables\|Seleccioneu aquí el color de fons per a les hores laborables\|Seleccioneu aquí el color per a les hores laborables\|[Ss]eleccions\|sencera com\|subtasques\|tapades<\/span>\|temps\|temps actual\|[Uu]tilitzarà\|valor\|vista prèvia\) a la \(<[^<]\{1,\}>\|«\|\[\|\)vista\([^[:alnum:]]\)/\1 en la \2vista\3/g
...
# al mapa
s/\b\(Mostra una marca de telescopi\|solucionador\) al mapa\([^[:alnum:]]\)/\1 en el mapa\2/g
# al mateix
s/\b\(sortida\) al mateix\([^[:alnum:]]\)/\1 en el mateix\2/g
# al melic
...
# al mòdul
s/\b\(emmagatzema\|localitzadors de mailto\|resoldre imatges\) al mòdul\([^[:alnum:]]\)/\1 en el mòdul\2/g
 #
...
s/\b\([Ee]specifiqueu un camí absolut\|[Mm]ostrar\|[Mm]ostrarà\|[Oo]culta els elements nous\|selecciona[rt]\|seleccionada\) al quadre\([^[:alnum:]]\)/\1 en el quadre\2/g
...
# al sistema
s/\b\(existeix\|Llocs\|teniu coses prou recents\) al sistema\([^[:alnum:]]\)/\1 en el sistema\2/g
# al sufix
...
s/\b\(cursor\|fitxer\|imatge\|[Mm]ostra el fitxer\|[Mm]ostrar els vídeos\|mostrarà\|[Oo]bre\|ratolí\|rebudes\|s'obriran\|utilitzar\|utilitzarà\|veu\) al vis\(or\|ualitzador\)\([^[:alnum:]]\)/\1 en el visualitzador\3/g
# al vostre
s/\b\(aplicable\|configurada\|configurades\|configurat\|configurats\|emmagatzemant una galeta\|instal·lat el &frameworks;\|KDE\|referir-se a un recurs tal com un fitxer\|Responeu a missatges de text del telèfon\|tal com un fitxer\|Vegeu les notificacions del telèfon\) al vostre\([^[:alnum:]]\)/\1 en el vostre\2/g
# al wiki
