...
s/\b\([Cc]erca\|[Cc]erca (res)\|[Cc]erca %1\|triar paràmetres\|veu més habitualment\) a \([Ii]\)nternet\([^[:alnum:]]\)/\1 en Internet\3/g
# a invent
...
s/\() i\|astrometry.net»\|funcionava\|KStars\|per primera vegada\|Wayland\) a \(<[^<]\{1,\}>\|«\|\[\|\)Linux\([^[:alnum:]]\)/\1 en \2Linux\3/g
...
s/\b\(ANSVR\|AppData\|esmenes d'errors\|implementació per a la cadena d'eines llvm-mingw\|JPEG-XL\) a Windows\([^[:alnum:]]\)/\1 en Windows\2/g
# a Xplanet
...
# a l'habitació
s/\b\(en una màquina\) a l'habitació\([^[:alnum:]]\)/\1 en l'habitació\2/g
# a l'hemisferi
...
s/\b\(Inicia automàticament el temporitzador d'aparcament\|Mantén\) a l'inici\([^[:alnum:]]\)/\1 en l'inici\2/g
# a l'insegur
...
# a la captura
s/\b\(4656x3520\|exclou les decoracions\|exclou les ombres\|inclou el punter\|[Mm]ostra\|mostra't\|Per exemple,\|s'inclou\|s'inclouen\|tal com s'il·lustra\) a la captura\([^[:alnum:]]\)/\1 en la captura\2/g
# a la cara
...
s/\b\(existent\|Substituït amb &DBus;\) a la KDE\([^[:alnum:]]\)/\1 en la KDE\2/g
# a la LaKademy
...
# a la màquina
s/\b\(iniciar un servidor INDI\|instal·lar-lo\) a la màquina\([^[:alnum:]]\)/\1 en la màquina\2/g
# a la marca
...
# al disc
s/\b\(apuntar cap a un estil\|canviï\|capturades\|dades\|desa[rt]\|desaran\|desin\|desin les imatges de l'enfocament automàtic\|dispositiu\|emmagatzemant-les\|emmagatzemats\|emmagatzemen\|Error durant l'escriptura\|escenes\|escriptura\|espai\|imatges\|memòria cau\) al disc\([^[:alnum:]]\)/\1 en el disc\2/g
# al dispositiu
s/\b\(activat\|desactivat\|desaran\|sector %1\) al dispositiu\([^[:alnum:]]\)/\1 en el dispositiu\2/g
 #
...
s/\b\(apareixerà\|canvieu al mode de Coordenades equatorials\|cercar-la\|de manera que l'opció\|[Ii]nformeu de tots els errors\|nacional\|traducció,\|trobeu cap documentació\|unió\) al \(<[^<]\{1,\}>\|«\|\[\|\)lloc\([^[:alnum:]]\)/\1 en el \2lloc\3/g
 #
...
# al porta-retalls
s/\b\(certificat\|[Cc]opia\|Copia el primer element\|copia la imatge\|copiar la imatge\|dades útils\|dades vàlides\|de la imatge\|ha actualment\|ha dades\|ha informació\|hi ha\|hi ha cap dada vàlida de fotograma clau\|hi ha cap element\|pantalla\|posa'l\|si n'hi ha,\|[Ss]ubstitueix\|text\) al porta-retalls\([^[:alnum:]]\)/\1 en el porta-retalls\2/g
# al primer
...
# al servidor
s/\b\(acronym>\|deixar els missatges\|[Ee]rror intern\|iniciar la sessió\|obrir\|pertanyents a les claus de targeta intel·ligent\|utilitzada\) al \(<[^<]\{1,\}>\|«\|\[\|\)servidor\([^[:alnum:]]\)/\1 en el \2servidor\3/g
# al seu
...
# al sistema
s/\b\(en l'enfocament automàtic o\|existeix\|instal·lar l'aplicació\|Llocs\|suport d'àudio\|teniu coses prou recents\) al sistema\([^[:alnum:]]\)/\1 en el sistema\2/g
 #
...
s/\b\(aplicable\|[Cc]erca\|configurada\|configurades\|configurat\|configurats\|des d'una llista de fonts\|desats\|emmagatzemant una galeta\|estiguessin\|executant\|hi ha\|instal·lada\|instal·lar el mòdul d'impressora\|instal·lat\|instal·lat el &frameworks;\|instal·lats\|KDE\|referir-se a un recurs tal com un fitxer\|Responeu a missatges de text del telèfon\|tal com un fitxer\|troba\|troben els controladors INDI\|Vegeu les notificacions del telèfon\) al vostre\([^[:alnum:]]\)/\1 en el vostre\2/g
# al wiki
